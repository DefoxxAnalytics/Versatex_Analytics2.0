...
export function useFilterPresets() {
   ...
   * Save a new preset
   */
  const savePreset = useCallback((name: string, filters: Filters): FilterPreset => {
   ...
   */
  const updatePreset = useCallback((id: string, updates: Partial<Pick<FilterPreset, 'name' | 'filters'>>): void => {
  ...
  const nameExists = useCallback((name: string, excludeId?: string): boolean => {
    return presets.some((p) => p.name.toLowerCase() === name.toLowerCase() && p.id !== excludeId);
  }, [presets]);
